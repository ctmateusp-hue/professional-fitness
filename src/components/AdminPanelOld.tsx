...
import { FaArrowLeft, FaUpload, FaTrash, FaPlus, FaSave, FaImage, FaPlay, FaLink, FaFile, FaUsers, FaSignOutAlt, FaStar } from 'react-icons/fa'
...
export function AdminPanel({ onBack, modalities }: AdminPanelProps) {
  const storageHook = usePersistentStorage()
  const { data, saveData } = storageHook
  const { media } = data
  ...
  const addMedia = async () => {
    ...
    try {
      ...
      // Add media using the storage service
      const newMedia = {
